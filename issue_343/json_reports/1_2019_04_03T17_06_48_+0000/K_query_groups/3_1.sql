-- queryid: 2627010232312073700
-- NOTICE: the first 50k characters
-- NOTICE: current query size (bytes): '9827'
with fk_indexes as (
 select
 ...
 *,
 ...
 coalesce(nullif(quote_ident(tnsp.nspname), $56) || $57, $58) || quote_ident(trel.relname) as formated_relation_name,
 i2.opclasses
 from
 index_data as i1
 ...
 inner join pg_opclass op1 on i1.indclass[$59] = op1.oid
 inner join pg_opclass op2 on i2.indclass[$60] = op2.oid
 ...
 and i1.columns like (i2.columns || $61) 
 and i1.opclasses like (i2.opclasses || $62)
 ...
 select
 ri.*,
 case when fi.index_name is not null then $63 else $64 end as supports_fk
 from redundant_indexes ri
 left join fk_indexes fi on
 fi.fk_table_ref = ri.table_name
 and fi.opclasses like (ri.opclasses || $65)
 ...
 index_name,
 access_method,
 string_agg(reason, $66) as reason,
 string_agg(main_index_def, $67) as main_index_def,
 string_agg(main_index_size, $68) as main_index_size,
 index_def,
 index_size_bytes,
 index_usage,
 formated_index_name,
...
redundant_indexes_json as (
 select
 json_object_agg(rin.schema_name || $70 || rin.index_name, rin) as json
...
together as (
 ...
 select format($72, max(formated_index_name), max(index_size), string_agg(reason, $73), table_name) as line
 from together t1
 ...
 select
 replace(
 format($74, max(index_def), table_name),
 $75,
 $76
 )as line
 from together t2
 ...
 age(
 date_trunc($77,now()),
 date_trunc($78,sd.stats_reset)
 ) as stats_age,
 ((extract($79 from now()) - extract($80 from sd.stats_reset))/$81)::int as days
