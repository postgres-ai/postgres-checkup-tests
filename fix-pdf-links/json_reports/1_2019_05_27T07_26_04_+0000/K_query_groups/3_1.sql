-- queryid: 8681280274376157000
-- NOTICE: the first 50k characters
-- NOTICE: current query size (bytes): '6392'
with data as (
 ...
 *,
 ...
 then $66 || pg_size_pretty(extra_size::numeric)::text || $67 || round(extra_ratio::numeric, $68)::text || $69
 else $70
 end as "extra",
 case
 when extra_size::numeric >= $71
 then extra_size
 else $72
 end as "extra_size_bytes",
 case
 when bloat_size::numeric >= $73
 then $74 || pg_size_pretty(bloat_size::numeric)::text || $75 || round(bloat_ratio::numeric, $76)::text || $77
 else $78
 end as "bloat",
 case
 when (bloat_size)::numeric >=$79
 then bloat_size
 else $80
 end as "bloat_size_bytes",
 case
 when (bloat_ratio)::numeric >=$81
 then bloat_ratio
 else $82
 end as "bloat_ratio_percent",
 case
 when (real_size - bloat_size)::numeric >=$83
 then $84 || pg_size_pretty((real_size - bloat_size)::numeric)
 else $85
 end as "live_data_size",
 case
 when (real_size - bloat_size)::numeric >=$86
 then (real_size - bloat_size)::numeric
 else $87
 end as "live_data_size_bytes",
 case
 when (real_size - bloat_size)::numeric >=$88
 then (real_size - bloat_size)::numeric
 else $89
 end as "live_bytes",
 fillfactor,
 case when ot.table_id is not null then $90 else $91 end as overrided_settings,
 table_size_bytes
 from step4
 ...
 (sum("bloat_size_bytes")::numeric/sum("real_size_bytes")::numeric * $93) as "bloat_ratio_percent_avg",
 sum("extra_size_bytes") as "extra_size_bytes_sum",
...
select
 ...
 $96,
 (select count($97) from limited_data where overrided_settings = $98)
 )
