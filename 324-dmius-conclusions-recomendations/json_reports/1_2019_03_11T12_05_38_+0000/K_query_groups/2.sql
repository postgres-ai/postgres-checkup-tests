 ...
 * extract($31 from now() - stats_reset)
 ...
 round((temp_files / (((extract($63 from now()) - extract($64 from data.stats_reset))/$65)))::numeric, $66)::text
 else
 temp_files::text
 end
 from data
 union all
 select $67, pg_size_pretty(temp_bytes::numeric / nullif(temp_files, $68))::text from data
 union all
 select $69, deadlocks::text from data
 union all
 select
 $70,
 case
 when ((extract($71 from now()) - extract($72 from data.stats_reset))/$73)::int <> $74 then
 round((deadlocks / (((extract($75 from now()) - extract($76 from data.stats_reset))/$77)))::numeric, $78)::text
 else
 deadlocks::text
 end
 from data
), general_info_json as (
 select json_object_agg(data.metric, data) as json from data where data.metric not like $79
), database_sizes as (
